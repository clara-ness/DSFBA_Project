...
However, in terms of total calorie intake it is also the *.th* cluster that consumes the most calories. One might therefore think that calorie consumption is not the main reason why high-GDP countries have lower diabetes rates.
...
row.names(cl3)[12] <- ("Means")
kable(cl3[12,])
```
As we see from the cluster averages, cluster 2 has the highest rate of diabetes followed by cluster 3 and finally the cluster with the lowest rate of diabetes. 
```{r}
slices <- c(cl1[3,5],cl1[3,6],cl1[3,7], cl1[3,8] )
...
slices <- c(cl3[12,5],cl3[12,6],cl3[12,7], cl3[12,8] )
lbls <- c("Animal Protein", "Plant Protein", "Carbs", "Fat")
pct <- round(slices/sum(slices)*100)
lbls <- paste(lbls, pct) # add percents to labels
lbls <- paste(lbls,"%",sep="") # ad % to labels 
