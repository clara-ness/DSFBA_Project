...
We will now look in more detail at the relationship of each macronutrient to diabetes, starting with animal protein.

### Diabetes - Animal protein

In the EDA, we observe a decreasing trend for the animal protein related to diabetes. We are going to verify this in the correlation. 
```{r}
#Diabetes - calories
...
```
The trend is verify here with a negative trend for both men women, we can also see that the correlation is higher for women. The linear regression does not indicate any relation, so we will try to use the proportion of animal protein. 
...
```
The correlation is now higher for men. Linear regressions are not significant. We will now see if there is a relationship between diabetes and animal protein for each cluster.
...
```
For the first cluster, we would expect to have a positive relationship based on the cluster graph. However, both the correlations and the linear regressions indicate a relationship that is more negative. It is also important to take into account that the regressions are not siginificant and it is therefore hard to draw anything from them. 

Regarding the second cluster, we should expect a negative relationship but it seems to be weak and negative.

For the last cluster, the relationship seems to be more or less high and positive. It is also hard to conclude a relationship for cluster 3 because the regressions are not significant.


### Diabetes - Plant protein

We saw at the EDA section, that there seemed to be a trend between these 2 variables. The calories consumed from plant protein is increasing as well as the diabetes. To confirm this, we will check the correlation.
...
As we can see, the correlation is very low and when we try to fit a linear model over these 2 variables, the parameter estimation is not significant at all and the parameters are equal to 0 for both.
Can we find a better explanation by using the proportion of calories rather than the calorie count of the plant protein consumed ?
...
As expected the correlation are higher for both gender and the linear regression shows that the plant protein is highly significant for women. 
We will now look at the relationship of the plant protein to each cluser.
```{r}
#cluster 1
...
```
For the first cluster, according to the cluster graphs, we would expect to see a rather negative relationship but the correlations show us a rather high correlation for men and even higher for women.

The second cluster should show a positive relation but we can see that the correlation are positive for men and negative for women.

Given the output for the third cluster, we cannot conclude anything.

### Diabetes - Carbohydrates
In the eda part, it seems that carbohydrates have a rather flat trend compared to diabetes. So we will see how they correlate.
```{r}
#Diabetes - calories
...
```
The correlation is quite small for the women and it is positive, whereas the men have also a small correlation but it is negative. We will now see if we can get more information from the proportions.
...
```
The correlation is now higher for both genders and it should be noted that women even have a fairly high correlation (0.616).Moreover, the linear regression for women is significant, so increasing carbohydrates in women would increase their diabetes rate. 
```{r}
#cluster 1
...
```
For the first cluster, one would expect a rather negative relationship but the correlation is weak and positive.

For the second cluster, one would expect to have a positive relationship which is the case for men but for women. 
We cannot conclude anything unfortunately for the 3rd cluster even if we will expect to have a rather negative relationship. 

### Diabetes - Fats
In the EDA part, the fats have a rather downward trend. We will now explore their relationship with diabetes by looking at the correlation.
...
```
The correlation is negative for both genders and again the correlation is greater for women than for men. We are now going to see if we can see clearer with the proportions. 
```{r}
#Diabetes - proportion
cor(GDP_diabetes_cal$prop_men_diabetes,GDP_diabetes_cal$proportion_fat)
cor(GDP_diabetes_cal$prop_women_diabetes,GDP_diabetes_cal$proportion_fat)
...
```
The correlation with the proportion is more or less the same. The fat is also highly significant for women even though it only adds a little increase to the diabetes rate. We are now going to see the relation with the cluster. 
...
```
For the first cluster, we expect to have a positive relationship but the correlations are very weak and it is hard to conclude anything.
For the second cluster, we should have a rather negative relationship which is the case for women but not for men. 
For the third cluster, we should have a rather positive relationship which is not the case for the correlations. Moreover, the regressions are not significant so we cannot conclude anything.

### Typical Diet by Clusters
We will now discuss for each cluster their diet patterns. 
```{r}
#proportion animal protein
