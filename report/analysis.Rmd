...
leaflet(SHP_0) %>%
  ...
  opacity = .6)
```
We can therefore see that the first cluster is more located in the north and center of Europe while the second cluster is more towards the extremities of Europe (West and East). We can then ask ourselves what are the characteristics of the eastern and western countries. For this we will look at how diabetes is distributed in European countries.
...
```
Women with diabetes would therefore be mainly located in the Eastern countries and also in the South-Western countries, which corresponds to the cluster 2 that seems to have the most diabetes. 
<br><br>
```{r}
pal <- colorNumeric(
  palette = "YlGnBu",
...
```
We can see then that the countries that are in cluster 2 have lower GDP. 
We will nevertheless verify this statement algebraically by looking at the characterizations of the clusters with kmeans.  
<br><br>
```{r, warning=FALSE}
