...
We saw at the EDA section, that there seemed to be a trend between these 2 variables. The calories consumed from plant protein is increasing as well as the diabtes. To confirm this, we will check the correlation.
...
##### Change this!The cluster plot above indicates that in countries with higher GDPs like in cluster 1 and 3, the proportions of calories consumed from plant protein are lower than in countries with lower GDPs. Let's see if we can confirm these observations and see if these relationships are strong enough within each cluster.

```{r}
#cluster 1
cor(countries_cluster_1bi$prop_men_diabetes, countries_cluster_1bis$proportion_plant_prot)
cor(countries_cluster_1bi$prop_women_diabetes, countries_cluster_1bis$proportion_plant_prot)

lm_diabm_prop_plant_prot_clust1 <- lm(countries_cluster_1bi$prop_men_diabetes ~ countries_cluster_1bis$proportion_plant_prot)
lm_diabw_prop_plant_prot_clust1 <- lm(countries_cluster_1bi$prop_women_diabetes ~ countries_cluster_1bis$proportion_plant_prot)
