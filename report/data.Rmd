...
* Lastly, we computed the average GDP for each country over the years 2000-2013 in order to be able to plot different variables together. (We also decided to scale our data)
...
### Diabetes
The third dataset, from https://www.ncdrisc.org/data-downloads-diabetes.html, provides us information about the age-standardised diabetes prevalence for each country and sex/gender from 1980 to 2014.
...
And a variable for each year from 1980 to 2014.
...
* We also want to keep only european countries.
...
```
* Now we will separate our dataset in two subset : 
* one with data about men :
...
GDP_diabetes_cal[,3:10] <- scale(GDP_diabetes_cal[,-c(1,2)]) #attempt of scaling the whole table
```
