...
We now have a dataframe with the following variables :
  ...
  + `country_name` name of the country  
  + `country_code` ISO code of the country  
  + `avg_population` the population of a country over the course of 2000-2013
  
In order to compare the GDP per person and calorie consumption per person, we will create a separate table.
...
GDP_diabetes_cal_pp <-full_join(GDP_diabetes_cal, GDP_summarized2, by="country_code")

kbl(GDP_diabetes_cal,
    caption = "**Finale Dataset**: GDP, diabetes and calories",
    col.names = c('Country Name','Country Code','Average GDP (in billion $)','Men Diabetes', 'Women Diabetes', 'Calories from animal protein', 'Calories from plant protein','Calories from carbohydrates', 'Calories from fat', 'Total consumption'),
    align = c('cccccc')) %>%
   kable_classic(full_width = T, html_font = "Cambria") %>%
   kable_styling(bootstrap_options = c('striped', 'hover', 'condensed'))
```
